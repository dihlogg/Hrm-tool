...
export function useGetMyPendingRequests(employeeId: string, page: number = 1, pageSize: number = 5) {
  ...
  useEffect(() => {
    ...
    async function loadRequests(employeeId: string) {
      ...
      try {
        ...
        >(`${API_ENDPOINTS.GET_MY_PENDING_REQUESTS}/${employeeId}`, {
          params,
        });
      ...
      const errorObj = err as { response?: { data?: { message?: string } }; message?: string };
        setError(errorObj.message || "Failed to load my pending requests");
      } finally {
        setLoading(false);
      }
    }
