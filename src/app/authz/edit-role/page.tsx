...
import { CreateRoleDto } from "@/hooks/roles/CreateRoleDto";
...
export default function EditRolePage() {
  ...
  const roleId = searchParams.get("id") || undefined;
  const { role } = useGetRoleById(roleId ?? "");

  useEffect(() => {
    if (role) {
      setName(role.name ?? "");
      setDescription(role.description ?? "");
      setDisplayOrder(role.displayOrder ?? 0);
    }
  }, [role]);
