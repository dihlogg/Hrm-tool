...
import { RoleDto } from "@/hooks/roles/CreateRoleDto";
import { useGetAllRoles } from "@/hooks/roles/useGetAllRoles";
import { useDeleteRoleById } from "@/hooks/roles/useDeleteRoleById";
...
export default function RolePage() {
  ...
  const { roles } = useGetAllRoles(hotReload);
    ...
    if (selectedRole?.id) {
      try {
        ...
        api.success({
          ...
          description: `Role information has been deleted.`,
        ...
        api.error({
          ...
          description: "Role information has not been deleted",
          placement: "bottomLeft",
          duration: 3,
          pauseOnHover: true,
        });
  ...
  const columns: ColumnsType<RoleDto> = [
    ...
    {
      title: <span className="select-none">Role Name</span>,
      dataIndex: "name",
      ...
      render: (_: unknown, record: RoleDto) => (
        <div className="flex gap-4">
          <Button
            ...
            shape="circle"
            onClick={() =>
              router.push(`/authz/edit-role?id=${record.id}`)
            }
    ...
    <>
      ...
      >
        ...
        <p className="mt-2 text-sm text-gray-500">
          This action will delete the role from the system!
        </p>
      </Modal>
    </>
  );
