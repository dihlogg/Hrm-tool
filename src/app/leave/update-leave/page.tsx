...
export default function UpdateLeaveRequestPage() {
  ...
  const [expectedApproverId, setExpectedApproverId] = useState<
    string | undefined
  >(undefined);
  const [expectedConfirmId, setExpectedConfirmId] = useState<
    string | undefined
  >(undefined);
  const [expectedInformToId, setExpectedInformToId] = useState<
    string | undefined
  >(undefined);
  ...
  useEffect(() => {
    const selectedPartial = partialDays.find((p) => p.id === partialDayId);
    if (selectedPartial) {
      if (selectedPartial.name === "Cả Ngày") {
        setDuration("1.00");
      } else if (
        selectedPartial.name === "Nửa Ngày - Sáng" ||
        selectedPartial.name === "Nửa Ngày - Chiều"
      ) {
        setDuration("0.50");
      } else {
        setDuration("");
