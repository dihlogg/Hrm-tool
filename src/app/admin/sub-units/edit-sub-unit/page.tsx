...
import { useUpdateSubUnit } from "@/hooks/employees/sub-units/useUpdateSubUnit";
import { useGetSubUnitById } from "@/hooks/employees/sub-units/useGetSubUnitById";
import { useSearchParams } from "next/navigation";

export default function EditSubUnitPage() {
  ...
  const subUnitId = searchParams.get("id") || undefined;
  const { subUnit } = useGetSubUnitById(subUnitId ?? "");

  useEffect(() => {
    if (subUnit) {
      setName(subUnit.name ?? null);
      setDescription(subUnit.description ?? null);
    }
  }, [subUnit]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateSubUnit(subUnitId!, payload);
      console.log("Sub Unit updated successfully!");
      api.success({
        message: "Sub Unit Update Successfully!",
        description: `Sub Unit has been updated.`,
      ...
      api.error({
        message: "Sub Unit Update failed!",
        description: message,
        placement: "bottomLeft",
        duration: 3,
        pauseOnHover: true,
